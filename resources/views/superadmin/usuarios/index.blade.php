...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
        <div class="flex items-center">
            ...
            <div>
                ...
                <p class="text-gray-600 text-sm">Gestiona los usuarios del sistema y sus permisos</p>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        @php
            $totalAdmins = $usuarios->count();
            $activos = $usuarios->where('estadoUsu', 'activo')->count();
            $inactivos = $usuarios->where('estadoUsu', 'inactivo')->count();
            $conPermisos = $usuarios->filter(function($admin) {
                return $admin->permissions->count() > 0;
            })->count();
        @endphp
        
        @foreach([
            'total' => ['title' => 'Total Usuarios', 'color' => 'blue', 'icon' => 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0z'],
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($usuarios as $admin)
                            <tr class="hover:bg-gray-50 transition duration-150 ease-in-out">
                                        ...
                                        <div class="flex-shrink-0 h-12 w-12">
                                            <div class="h-12 w-12 rounded-full bg-blue-100 flex items-center justify-center border-2 border-white shadow-sm">
                                                <span class="text-blue-600 font-semibold text-sm">
                                                    {{ strtoupper(substr($admin->nomUsu, 0, 1)) }}{{ strtoupper(substr($admin->apeUsu, 0, 1)) }}
                                                </span>
                                            </div>
                                        </div>
                                        <div class="ml-4">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ $admin->emaUsu }}</div>
                                    <div class="text-sm text-gray-500">{{ $admin->telUsu ?: 'Sin teléfono' }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ $admin->TipdocUsu }}</div>
                                    <div class="text-sm text-gray-500">{{ $admin->numdocUsu ?: 'Sin documento' }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-200">
                <div class="flex flex-col md:flex-row items-center justify-between">
                    <div class="text-sm text-gray-500 mb-4 md:mb-0">
                        Mostrando <span class="font-medium">{{ $usuarios->firstItem() }}</span> a <span class="font-medium">{{ $usuarios->lastItem() }}</span> de <span class="font-medium">{{ $usuarios->total() }}</span> administradores
                    </div>
                    {{ $usuarios->links() }}
                </div>
            </div>
